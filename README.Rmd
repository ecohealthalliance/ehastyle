...
The functions in this package are generally used in the `yaml` of rmarkdown documents. 
...
You can use `rmarkdown::draft` to create files from the templates in the package. 

```{r example draft, eval=FALSE}
...
```{r templates, echo=FALSE, results='asis'}
templates <- list.dirs("./inst/rmarkdown/templates/",full.names = FALSE,recursive = F)

glue::glue("- {templates}",.sep = "\n")
...
knitr::include_graphics("./man/figures/openRMD.png")
```

- Click on the **from template** option

```{r click, echo=FALSE, out.width = '100%'}
knitr::include_graphics("./man/figures/clickTemplate.png")
```

- Select the template you would like to use
```{r select, echo=FALSE, out.width = '100%'}
knitr::include_graphics("./man/figures/selectTemplate.png")
```

- Modify the template
```{r modify, echo=FALSE, out.width = '100%'}
knitr::include_graphics("./man/figures/nameDash.png")
```

- Knit and save your content
```{r save, echo=FALSE, out.width = '100%'}
knitr::include_graphics("./man/figures/saveRMD.png")
```
...
This package contains `classic`, `avenir`, and `predict` presentation templates that can be used with an aspect ratio of `16x9` or `4x3` to generate ppt or html based slides. 

Functions follow this form `eha_template_output`. For example`eha_avenir_xar` will generate html based slides using the [{xaringan}](https://bookdown.org/yihui/rmarkdown/xaringan.html) package and the avenir font.    

`classic` will always contain the standard slide theme as defined by our communications professionals. 

`avenir` will contain a variation of `classic` that uses the avenir font. 

`predict` contains a legacy template from EcoHealth Alliance's work with USAID
predict. 
...
This package currently contains a single function and template for creating [{flexdashboard}](https://rstudio.github.io/flexdashboard/index.html) rmd documents.

`Flexdashboards` allow you to create dashboards with ease. You can embed static or
dynamic plots or elements without having to learn rshiny. 

## Hosting Fonts on AWS

The Avenir font used in the different templates is currently hosted on AWS instance in an S3 bucket called `font-avenir`. There are two files, one is a woff2 font file and the other is a css document describing the font face. This workflow could be adapted for other templates that require a custom/proprietary font. 
